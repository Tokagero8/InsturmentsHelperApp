...
import com.example.insturmentshelperapp.presenter.lessons.LessonsChordSummaryPresenter;

import java.text.DateFormat;
import java.util.Calendar;

public class LessonsChordSummaryFragment extends Fragment implements LessonsChordSummaryInterface{

    LessonsChordSummaryPresenter presenter;
    boolean isPassed;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        return inflater.inflate(R.layout.fragment_lessons_chord_summary, container, false);
    }

    @Override
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        presenter = new LessonsChordSummaryPresenter(this, getActivity());
        ...
        Button addLessonButton = getView().findViewById(R.id.button_chord_add_lesson);
        if(addLessonButton != null){
            addLessonButton.setOnClickListener(addLessonButtonListener);
        }
        Button notAddLessonButton = getView().findViewById(R.id.button_chord_dont_add_lesson);
        if(notAddLessonButton != null){
            notAddLessonButton.setOnClickListener(notAddLessonButtonListener);
        }
        Button manualAddLessonButton = getView().findViewById(R.id.button_chord_add_lesson_manual);
        if(manualAddLessonButton != null){
            manualAddLessonButton.setOnClickListener(manualAddLessonButtonListener);
        }
        Button endLessonButton = getView().findViewById(R.id.button_end_chord_lesson);
        endLessonButton.setOnClickListener(endLessonButtonListener);
    }

    @Override
    public void setScore(String score) {
        TextView scoreText = getView().findViewById(R.id.text_view_chord_final_score);
        scoreText.setText(score);
    }

    @Override
    public void setNeededScore(int neededScore, double lessonIntensityMultipler) {
        TextView scoreText = getView().findViewById(R.id.text_view_chord_needes_score);
    ...
    public void setResult(int score, int neededScore, double lessonIntensityMultipler) {
        TextView resultText = getView().findViewById(R.id.text_view_chord_result);
        int finalNeededScore = (int) (neededScore * lessonIntensityMultipler);
    ...
    public void setNextLesson(Lesson lesson, int repeatability) {
        ...
        if(lesson == null){
            ...
            Button addButton = getView().findViewById(R.id.button_chord_add_lesson);
            ViewGroup viewGroup = (ViewGroup) addButton.getParent();
            viewGroup.removeView(addButton);

            Button dontAddButton = getView().findViewById(R.id.button_chord_dont_add_lesson);
            viewGroup = (ViewGroup) dontAddButton.getParent();
            viewGroup.removeView(dontAddButton);

            Button addManualyButton = getView().findViewById(R.id.button_chord_add_lesson_manual);
