...
import com.example.insturmentshelperapp.presenter.lessons.LessonsPlaySummaryPresenter;

import java.text.DateFormat;
import java.util.Calendar;


public class LessonsPlaySummaryFragment extends Fragment implements LessonsPlaySummaryInterface{

    LessonsPlaySummaryPresenter presenter;
    boolean isPassed;
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        return inflater.inflate(R.layout.fragment_lessons_play_summary, container, false);
    }

    @Override
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        presenter = new LessonsPlaySummaryPresenter(this, getActivity());
        ...
        Button addLessonButton = getView().findViewById(R.id.button_play_add_lesson);
        if(addLessonButton != null){
            addLessonButton.setOnClickListener(addLessonButtonListener);
        }
        Button notAddLessonButton = getView().findViewById(R.id.button_play_dont_add_lesson);
        if(notAddLessonButton != null){
            notAddLessonButton.setOnClickListener(notAddLessonButtonListener);
        }
        Button manualAddLessonButton = getView().findViewById(R.id.button_play_add_lesson_manual);
        if(manualAddLessonButton != null){
            manualAddLessonButton.setOnClickListener(manualAddLessonButtonListener);
        }
        Button endLessonButton = getView().findViewById(R.id.button_end_play_lesson);
        endLessonButton.setOnClickListener(endLessonButtonListener);
    }

    @Override
    public void setScore(String score) {
        TextView scoreText = getView().findViewById(R.id.text_view_play_final_score);
        scoreText.setText(score);
    }

    @Override
    public void setNeededScore(int neededScore, double lessonIntensityMultipler) {
        TextView scoreText = getView().findViewById(R.id.text_view_play_needes_score);
    ...
    public void setResult(int score, int neededScore, double lessonIntensityMultipler) {
        TextView resultText = getView().findViewById(R.id.text_view_play_result);
        int finalNeededScore = (int) (neededScore * lessonIntensityMultipler);
    ...
    public void setNextLesson(Lesson lesson, int repeatability) {
        ...
        if(lesson == null){
            ...
            Button addButton = getView().findViewById(R.id.button_play_add_lesson);
            ViewGroup viewGroup = (ViewGroup) addButton.getParent();
            viewGroup.removeView(addButton);

            Button dontAddButton = getView().findViewById(R.id.button_play_dont_add_lesson);
            viewGroup = (ViewGroup) dontAddButton.getParent();
            viewGroup.removeView(dontAddButton);

            Button addManualyButton = getView().findViewById(R.id.button_play_add_lesson_manual);
